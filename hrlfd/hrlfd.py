...
from hrlfd.models import ControllerActor, ControllerCritic, \
    ManagerActor, ManagerCritic
from hrlfd.config import Config
...
class Manager(object):
    def __init__(self, state_dim, goal_dim, action_dim, actor_lr,
        ...
        self.reachable_state = 300
        self.reachable_subgoal = 25
        self.positive_signal =8
        self.negative_signal =-2
    ...
    def train(self, controller_policy, replay_buffer, iterations, batch_size=100, discount=0.99,
        ...
        demo_state = np.load(r"AntMaze_state_demo_223.npy")
        demo_subgoal = np.load(r"AntMaze_subgoal_demo_223.npy")
        demo_batch_size = demo_state.shape[0]
        ...
        for it in range(iterations):
            ...
            for sz in range(demo_batch_size):
                demo_s = demo_state[sz]
                demo_s_tensor = torch.tensor(demo_s).to(device)
                demo_s = demo_subgoal[sz]
                demo_sg_tensor = torch.tensor(demo_s).to(device)
                pdist = nn.PairwiseDistance(p=2)
                state_dis = pdist(demo_s_tensor, state)
                state_dis = state_dis.sum()
                subgoal_dis = pdist(demo_sg_tensor, subgoal)
                subgoal_dis = subgoal_dis.sum()
                if state_dis<= self.reachable_state and subgoal_dis<= self.reachable_subgoal:
                    subgoal_flag=1
            ...
            if subgoal_flag !=1:
                actor_loss = actor_loss + self.positive_signal
            else:
                actor_loss = actor_loss + self.negative_signal
...
class Controller(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005):
        avg_act_loss, avg_crit_loss = 0., 0.
        state_demo = np.load(r"AntMaze_state_demo_223.npy")
        subgoal_demo = np.load(r"AntMaze_subgoal_demo_223.npy")
        action_demo = np.load(r"AntMaze_action_demo_223.npy")
        ...
        for it in range(iterations):
            ...
            Equal_Action = False
            fixed_constant = 0
            # determine it is smaller than demo_batchsize
            if it < demo_batchsize:
                demo_s = state_demo[it]
                demo_s_tensor = torch.tensor(demo_s).to(device)
                demo_a = action_demo[it]
                demo_sg = subgoal_demo[it]
                demo_sg_tensor = torch.tensor(demo_sg).to(device)
            if demo_a.all() == u.all():
                Equal_Action = True

            # no matter similarity, we calculate the distance of observation and demonstration still
            pdist = nn.PairwiseDistance(p=2)
            s_distance = pdist(demo_s_tensor,state)
            s_distance = s_distance.sum()
            sub_distance = pdist(demo_sg_tensor, sg)
            sub_distance = sub_distance.sum()
            ...
            target_Q = torch.min(target_Q1, target_Q2)
            target_Q = reward + (done * discount * target_Q)
            target_Q = (reward + fixed_constant * (s_distance + sub_distance) / 1000) + (done * discount * target_Q)
            target_Q_no_grad = target_Q.detach()
