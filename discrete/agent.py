...
from env import MazeEnv, KeyChestEnv, AntMazeSparseEnv
...
class Agent:
    ...
    def __init__(self, args):
        ...
        if args.env_name == 'Maze':
            ...
            self.eval_env = KeyChestEnv(step_limit=500, random_start=False, seed=args.seed+100)
            self.x_range = self.env.h
            self.y_range = self.env.w
            self.action_scale_l = None
            ...
            self.env = AntMazeSparseEnv(step_limit=500, random_start=True, seed=args.seed)
            self.eval_env = AntMazeSparseEnv(step_limit=500, random_start=False, seed=args.seed+100)
            self.x_range = self.env.h
            self.y_range = self.env.w
            self.action_scale_l = None
            self.total_training_frames = 5e6
            self.random_start = False
        else:
    ...
    def train_one_episode(self):
        ...
        if self.replay_buffer_h.start():
            ...
            self.loss_h = self.loss_policy_h + self.loss_value_h
    ...
    def train_high_level(self):
        ...
        # import demo
        state_demo = np.load(r"Maze_state_demo_3.npy")
        subgoal_demo = np.load(r"Maze_subgoal_demo_3.npy")
        demo_batch_size = state_demo.shape[0]
        state_dis_sum, subgoal_dis_sum = 0., 0.
        ...
        # Calculate the distance between observation and demonstration
        subgoal_flag = 0
        for sz in range(demo_batch_size):
            demo_s = state_demo[sz]
            demo_s_tensor = torch.from_numpy(demo_s).float().to(self.device)
            demo_s = subgoal_demo[sz]
            demo_sg_tensor = torch.from_numpy(demo_s).float().to(self.device)
            state_dis = torch.clamp(F.pairwise_distance(demo_s_tensor, states), min=0.).mean()
            state_dis_sum += state_dis
            subgoal_dis = torch.clamp(F.pairwise_distance(demo_sg_tensor, goals), min=0.).mean()
            subgoal_dis_sum += subgoal_dis
            if state_dis.sum() <=30:
                if subgoal_dis.sum()<=3:
                    subgoal_flag = 1
        ...
        loss_value_h = F.mse_loss(vals_1, ref_vals) + F.mse_loss(vals_2, ref_vals)
        ...
        if subgoal_flag == 1:
            loss_actor_h *= (state_dis_sum + subgoal_dis_sum)*-0.75
            #loss_actor_h *= (state_dis_sum ) * -0.75
        else:
            loss_actor_h *= (state_dis_sum + subgoal_dis_sum)*1.75
            #loss_actor_h *= (state_dis_sum ) * 1.75
    ...
    def train_low_level_a2c(self):
        ...
        state_demo = np.load(r"Maze_state_demo_3.npy")
        action_demo = np.load(r"Maze_action_demo_3.npy")
        subugoal_demo = np.load(r"Maze_subgoal_demo_3.npy")
        demo_batchsize = state_demo.shape[0]
        ...
        demo_s = torch.tensor(state_demo).to(self.device)
        demo_sg = torch.tensor(subugoal_demo).to(self.device)
        for i in range(min(batch_size , demo_batchsize)):
            if action_demo[i]== actions[i]:
                Equal_Action = True
            else:
                Equal_Action = False
            dis1 = torch.clamp(pow(pow(demo_s[i][0]-states[i][0],2)+pow(demo_s[i][1]-states[i][1],2),0.5), min=0.).mean()
            dis2 = torch.clamp(F.pairwise_distance(demo_sg[i],goals[i]), min=0.).mean()
            if Equal_Action:
                fixed_constant = -0.25
                s_distance += dis1*fixed_constant
                sub_distance += dis2*fixed_constant
            else:
                fixed_constant = 0.5
                s_distance += dis1 * fixed_constant
                sub_distance += dis2 * fixed_constant
    ...
    def test_one_episode(self):
        ...
        while True:
            ...
            last_goal_change = last_goal.cpu()
            last_goal_array = last_goal_change.detach().numpy()
            subgoal_list.append(last_goal_array)
        ...
        """
        if reward==1:
            # print and restore demonstrations
            np.save(r"AntMazeSparse_state_demo_{}".format(int(reward)), state_list)
            np.save(r"AntMazeSparse_subgoal_demo_{}".format(int(reward)), subgoal_list)
            np.save(r"AntMazeSparse_action_demo_{}".format(int(reward)), action_list)
        """
